...
public class Aukcja
{
	...
	public void insertAukcja()
	{
		...
		String query = String.format("INSERT INTO t_aukcje(id_uzytkownika, id_przedmiotu, nazwa, data_rozpoczecia, data_zakonczenia, stan)" + " VALUES (%d, %d, '%s','%s','%s', '%c')", id_uzytkownika,
				przedmiot.getId(), nazwa, data_rozpoczecia, data_zakonczenia, stan);
	...
	{
		PostgreSQLJDBC pgsq = new PostgreSQLJDBC();
		String query = String.format("DELETE FROM t_przebicia WHERE id_aukcji=%d", id);
		pgsq.queryOpertaion(query);
		deleteAutomaty();
	}
	...
	{
		PostgreSQLJDBC pgsq = new PostgreSQLJDBC();
		String query = String.format("DELETE FROM t_automaty WHERE id_aukcji=%d", id);
		pgsq.queryOpertaion(query);
	}
